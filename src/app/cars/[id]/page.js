  ...
  return (
    <section>
      <div className="relative min-h-screen w-full text-white px-6 md:px-20 py-20 flex flex-col items-center justify-center">
        ...
        {/* Image Slider */}
        <div className="w-full max-w-5xl mb-12 mt-8">
          <Swiper
            modules={[Navigation, Pagination, Mousewheel, Keyboard]}
            navigation
            pagination={{ clickable: true }}
            ...
            style={{ paddingBottom: "50px" }}
          >
            {[car.img, car.sideImg].filter(Boolean).map((imgUrl, index) => (
              <SwiperSlide key={index}>
                <div className="flex items-center justify-center h-[340px] sm:h-[420px] md:h-[520px] lg:h-[620px] bg-black rounded-2xl">
                  <img
        ...
        <div className="text-center mt-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 text-lg opacity-90">
            ...
            <div className="col-span-full mt-10">
              <Button action={"Book Now"} href={`/BookCar?id=${car.id}`} />
            </div>
          </div>
        </div>
      </div>
